 ...
 */
Map::Map(CompareFunc compare, DestroyFunc destroyKey, DestroyFunc destroyValue): compare(compare), destroyKey(destroyKey), destroyValue(destroyValue), size(0), deleted(0) {
...
/**
 * @brief Extend the hash table in case the load factor exceeds the predefined threshold
 * 
...
void Map::rehash() {
	...
	// Place in the new array only the entries that have a value (get rid of the deleted)
	this->size = 0;
...
void Map::insert(Pointer key, Pointer value) {
    ...
    int pos;
    for (pos = this->hash(key) % this->capacity;		            // Starting from the position of the x hash value
		this->array[pos]->getState() != EMPTY;						// If an empty cell is found, stopping the process
		pos = (pos + 1) % this->capacity) {					        // linear probing (if the end of the array is met, we start from the begining again)

        if (this->array[pos]->getState() == DELETED) {
			// A DELETED position is found. A new pair can be added here only if the key is new.
			// We keep the position and keep searching, because the key might be in one of the next cells
			if (node == nullptr)
				node = this->array[pos];

		} else if (this->compare(this->array[pos]->getKey(), key) == 0) {
			alreadyMember = true;
			node = this->array[pos];						        // Key is found, the new pair is added here (even if a DELETED position was found previously)
			break;											        // There is no need to continue searching
		}
	}
	if (node == nullptr)										        // If an EMPTY cell is found, node will not have a value yet
		node = this->array[pos];
	...
	if (alreadyMember == true) {
		...
		if (node->getState() == DELETED)							// If we found a DELETED state, we change it to OCCUPIED
			this->deleted--;
	}
	...
	// If after the insertion, the load factor threshold is surpassed we have to rehash
	// We take into account the DELETED nodes too, because they affect our computations
...
/**
 * @brief Remove from the map the pair (key --> value) that corresponds to the given key
 * 
...
bool Map::remove(Pointer key) {
	...
	// Set the state as DELETED, so not to stop the search but also make the insert easier
...
MapNode* Map::getNext(MapNode* node) {
    // node is a pointer to the i-th element of the array, so node - array[0] == i  (pointer arithmetic!)
...
MapNode* Map::findNode(Pointer key) {
	...
	int count = 0;
	for (int pos = this->hash(key) % this->capacity;		            // ξεκινώντας από τη θέση που κάνει hash το key
		this->array[pos]->getState() != EMPTY;				        // αν φτάσουμε σε EMPTY σταματάμε
		pos = (pos + 1) % this->capacity) {						    // linear probing, γυρνώντας στην αρχή όταν φτάσουμε στη τέλος του πίνακα

		// Μόνο σε OCCUPIED θέσεις (όχι DELETED), ελέγχουμε αν το key είναι εδώ
		if (this->array[pos]->getState() == OCCUPIED && this->compare(this->array[pos]->getKey(), key) == 0)
