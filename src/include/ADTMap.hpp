...
class MapNode {
	private:
		...
		State state;
	public:
		MapNode(Pointer key, Pointer value, State state): key(key), value(value), state(state) {};
		~MapNode() {};
...
class Map {
	...
	public:
		...
		void setDestroyValue(DestroyFunc destroyValue) {this->destroyValue = destroyValue;};
