...
TEST(ADTMapTest, insert) {
  ...
  delete[] valueArray;
	...
	map3->insert(&key1, &value1);			// Τα key1,key2 κάνουν hash στην ίδια τιμή (σε hash table μεγέθους 53)
...
TEST(ADTMapTest, iterate) {
  ...
  ASSERT_EQ(map->getFirst(), MAP_EOF);
	...
	int count = 0;
	for (MapNode* node = map->getFirst(); node != MAP_EOF; node = map->getNext(node)) {
